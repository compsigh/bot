...
if (!uri) throw new Error("Mongo URI environemnt variable is not defined")
...
export async function updatePetCount(petcount: number) {
  if (!petfaadilCollection) throw new Error("Failed to connect to database.")
...
export async function isBanned(userId: string) {
  if (!workingonBans) throw new Error("Failed to connect to database.")
