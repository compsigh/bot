  ...
  for (let dayOfWeek = 0; dayOfWeek < 7; dayOfWeek++) {
    for (const week of selectedWeeks) {
      if (dayOfWeek < week.contributionDays.length)
        contribGraph += getEmojiByFrequency(
          week.contributionDays[dayOfWeek].contributionCount
        )
    }
    contribGraph += "\n"
  }
    ...
    try {
      ...
      if (!contributionGraph) {
        await interaction.reply({
          content: `Couldn't find GitHub account "${githubUsername}", please double-check the username.`,
      ...
      await interaction.reply({
        content:
          "There was an error while fetching the contribution graph. Please try again later.",
        flags: MessageFlags.Ephemeral
      })
    }
  }
